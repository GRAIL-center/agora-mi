...
def main() -> int:
    ...
    parser.add_argument(
        "--tag",
        default="",
        help="Optional filename tag suffix (e.g., train/dev/test). Empty means no suffix.",
    )
    ...
    fdr_result = benjamini_hochberg(global_df["p_value"].values, q=args.q)
    ...
    print(f"Total global survivors (q < {args.q}): {len(survivors)}")

    print("\nSurvivors per layer:")
    layer_counts = survivors["layer"].value_counts().sort_index()
    for layer in args.layers:
        print(f"  Layer {layer}: {int(layer_counts.get(layer, 0))} features")

    if 24 in args.layers:
        l24_orig_count = 42
        l24_new_count = int(layer_counts.get(24, 0))
        print("\nLayer 24 comparison:")
        ...
        if l24_new_count == l24_orig_count:
            ...
            print(f"  Lost {l24_orig_count - l24_new_count} features under global correction.")
