...
def main() -> int:
    ...
    parser.add_argument("--layer", type=int, required=True, help="Layer of the extracted graph")
    ...
    logging.info(f"Loading ReplacementModel ({model_id})...")
    ...
    def get_prob(logits, tokens) -> float:
        ...
        lp_inc = _sequence_logprob_from_logits(logits, prompt_pos=prompt_pos, target_token_ids=[inc_ids[0]]).item()
        lp_res = _sequence_logprob_from_logits(logits, prompt_pos=prompt_pos, target_token_ids=[res_ids[0]]).item()
        m = max(lp_inc, lp_res)
